...
SELECT
  ...
  'TX' AS state_filter;   -- set to a 2-letter state code, or 'All' for all states
...
JOIN overall_rank AS o
  ON  
  d.issuer_id   = o.issuer_id
  AND 
  d.fips        = o.fips
  AND 
  d.metal_level = o.metal_level
JOIN fips_ra AS f
  ON  d.fips       = f.fips
...
WHERE
  issuer_avg_monthly_enrollment IS NULL
  AND
pm."Parent_Name" = 'CHRISTUS Health'
;
SELECT *
FROM public.issuer_level_detailed_enrollment
WHERE "State" = 'TX'
AND "AverageMonthlyEnrollment" IS NULL

-SELECT *
FROM public.issuer_level_detailed_enrollment
WHERE "State" = 'TX'
-- AND CAST("AverageMonthlyEnrollment" AS integer IS NULL- =SELECT *
FROM public.issuer_level_detailed_enrollment
WHERE "State" = 'TX'
AND CAST("AverageMonthlyEnrollment" AS integer) = 0===SELECT *
FROM public.issuer_level_detailed_enrollment
WHERE "State" = 'TX'
AND "AverageMonthlyEnrollment" = 'NULL'===-- ============================================================
-- Landscape Compare — Single Year (PostgreSQL)
-- Builds landscape_hpa for one business year.
-- Rates are for a 21-year-old.
-- ============================================================
...
-- Parameters: change these before running
-- ============================================================
DROP TABLE IF EXISTS params;CREATE TEMP TABLE params AS
SELECT
  '2024'  AS business_year,
  'TX' AS state_filter;-- set to a 2-letter state code, or 'All' for all states
...
-- documentation for limitation details.
-- ============================================================
DROP TABLE IF EXISTS fips_ra;CREATE TEMP TABLE fips_ra AS
SELECT DISTINCT
   t.fips::text AS fips
  ,t.state
  ,t.rate_area
